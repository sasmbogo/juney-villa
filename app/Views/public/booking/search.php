...
<a href="/booking/create/<?= e($villa['slug']) ?>?check_in=<?= e($checkIn ?? '') ?>&check_out=<?= e($checkOut ?? '') ?>&guests=<?= $guests ?>" class="btn btn-gold w-100">Book Now</a></div>
</div></div>
<?php endforeach; ?>
<?php if (empty($villas)): ?><div class="col-12 text-center py-5"><p class="text-muted">No villas available for your selected criteria.</p><a href="/booking" class="btn btn-outline-dark">Try Different Dates</a></div><?php endif; ?>
</div></div></section>
